...
router.post('/', requireAuth, requireRole('admin'), (req, res) => {
  const { name, sku, price, cost, stock, category_id, image_emoji } = req.body || {};
...
router.put('/:id', requireAuth, requireRole('admin'), (req, res) => {
  ...
  const fields = ['name', 'sku', 'price', 'cost', 'stock', 'category_id', 'image_emoji', 'active'];
  const updates = {};
  ...
  db.prepare(`
    UPDATE products SET name = ?, sku = ?, price = ?, cost = ?, stock = ?, category_id = ?, image_emoji = ?, active = ?
    WHERE id = ?
  `).run(
    merged.name,
    merged.sku,
